...
import { X, UserPlus, Pencil, Check, Loader2 } from 'lucide-react';
...
export const NewClientModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (clientToEdit) {
        ...
        setReferences(clientToEdit.references || '');
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-sm animate-in fade-in overflow-hidden"
      onClick={onClose}
    >
      <div
        id="client-modal-content"
        className="bg-[#111111] border border-[#1F1F1F] rounded-2xl w-full max-w-lg max-h-[90dvh] flex flex-col shadow-2xl overflow-hidden my-auto"
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                {isEditMode
                  ? `Expediente: ${clientToEdit?.firstName} ${clientToEdit?.lastName}`
                  : 'Expediente de Cliente'}
              </span>
              <h3 className="text-base font-bold text-white tracking-tight uppercase">
                {isEditMode ? 'Editar Datos del Cliente' : 'Registrar Nuevo Cliente'}
              </h3>
            </div>
          </div>
          <button
            ...
            className="p-1.5 text-zinc-400 hover:text-white bg-[#1A1A1A] border border-[#1F1F1F] rounded-lg transition-colors cursor-pointer disabled:opacity-50"
            title="Cerrar"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-4 bg-[#161616] border-t border-[#1F1F1F] flex items-center justify-end gap-3 shrink-0">
          <button
            ...
            onClick={onClose}
            disabled={isSubmitting}
            className="px-4 py-2.5 bg-[#161616] hover:bg-zinc-800 text-zinc-300 text-xs font-bold rounded-lg transition-colors cursor-pointer border border-[#1F1F1F] uppercase tracking-wider disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            form="client-form"
            disabled={isSubmitting}
            className="px-5 py-2.5 bg-[#22C55E] hover:bg-green-400 text-black text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50 uppercase tracking-wider shadow-lg shadow-green-950/40"
          >
            {isSubmitting ? (
              <>
